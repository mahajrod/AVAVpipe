rule baserecalibrator:
    ...
    log:
        "%s/{sample_id}/baserecalibrator.log" % log_dir
    benchmark:
    ...
    shell:
        "gatk BaseRecalibrator -R {input.reference} -I {input.bam} {input.known_sites_vcf_list} -O {output} > {log} 2>&1"
...
rule applybsqr:
    ...
    log:
        "%s/{sample_id}/applybsqr.log" % log_dir
    benchmark:
    ...
    shell:
        "gatk ApplyBQSR -R {input.reference} -I {input.bam} --bqsr-recal-file {input.table} -O {output} > {log} 2>&1"